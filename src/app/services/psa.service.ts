...
export class PsaService {
	...
	private serializePSAMedia(doc: Document): (PSAMedium | null)[] {
		return Array.from(
			{ ...doc.querySelectorAll('article'), length: 14 },
			(maybeArticle: HTMLDivElement | undefined) => {
				if (!maybeArticle) return null;

				return {
					id: maybeArticle.id.slice(5),
					name:
						maybeArticle.querySelector('.post-title > a')
							?.textContent ?? 'No name found!',
					thumbnail:
						maybeArticle.querySelector<HTMLImageElement>(
							'.post-thumbnail > a > img'
						)?.src ?? 'No thumbnail found!',
					fullLink:
						maybeArticle.querySelector<HTMLAnchorElement>(
							'.post-thumbnail > a'
						)?.href ?? 'No link found!',
					caption:
						maybeArticle.querySelector('.caption')?.textContent ??
						'No caption found!',
					excerpt:
						maybeArticle.querySelector('.excerpt > p')
							?.textContent ?? 'No excerpt found!',
					categories: [
						...maybeArticle.querySelectorAll('.post-category > a')
					].map((a) => a.textContent ?? '')
				};
			}
		);
	...
	private getReleaseSize(elm: HTMLElement): number {
		...
		return groups.byte === 'MB'
			? parseFloat(groups.size)
			: parseFloat(groups.size) * 1000;
	}
	...
	private getMovieReleaseInfo(elm: HTMLElement) {
		...
		const { groups } =
			/File size : (?<size>(?:\d+|\d+\.\d+)) (?<byte>MB|GB)/.exec(txt) ??
			{};
	...
	private createMovieRelease(elm: HTMLElement): PSAMovieRelease | null {
		...
		if (!info?.nextElementSibling || !name || /[\u0335-\u0337]/.test(name))
			return null;

		const { sizeMB, source, hasSubtitles } = this.getMovieReleaseInfo(
			info as HTMLElement
		);
	...
	private serializePSAShow(doc: Document): PSAShow {
		return {
			...
			releases: $$('.sp-wrap', doc)
				...
				.sort(
					(
						{ season: xs, episode: xe },
						{ season: ys, episode: ye }
					) => (ys - xs === 0 ? ye - xe : ys - xs)
				)
		};
	}
	...
	getMediaByCategory(
		category: PSACategory,
		page = 0
	): Observable<PSAMedium[]> {
		const cacheKey = `${category}-${page}`;
		if (this.CACHE[cacheKey]) return of(this.CACHE[cacheKey]);

		const uri = `https://${this.PSA_DOMAIN}/category/${category}/${
			page > 0 ? `page/${page + 1}/` : ''
		}`;
		...
		return this.getPSADocument(uri).pipe(
			map((doc) =>
				this.serializePSAMedia(doc).filter(
					(maybeMedium): maybeMedium is PSAMedium => !!maybeMedium
				)
			),
			tap((media) => {
	...
	updatePSADomain(): Observable<string> {
		return this.getPSADocument(
			`https://${this.PSA_DOMAIN}/psarips-active-domains-and-mirrors/`
		).pipe(
			map((doc) => {
				...
				if (
					strongTag?.textContent?.includes('Active') &&
					aTag?.textContent?.startsWith('https://')
				) {
