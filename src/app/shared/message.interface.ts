...
async function extractOuoIo(ouoioLink: string): Promise<string[]> {
	...
	const { data: ouoCaptcha2 } = await axiosInstance.post(
		formCaptcha.action,
		serializeForm(formCaptcha),
		{
	...
	const { data: resultData } = await axiosInstance.post(
		formGo.action,
		serializeForm(formGo),
		{
			headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
		}
	);
...
async function guess(releaseName: string): Promise<string[]> {
	...
	try {
		const { data: resultData } = await axiosInstance.get(
			`https://get-to.link/${kebab}999`
		);
	...
	for (let idx = 1; idx < 20; idx++) {
		const prom = cookies
			...
			.then((cj) =>
				cj
					.setCookie(
						`VstCnt=${Buffer.from(idx.toString()).toString(
							'base64'
						)}; path=/; secure`,
						psaUrl.origin
					)
			...
			.then((cj) =>
				axiosInstance.get<string>(exitLink, {
...
async function getOuoRedirect(exitLink: string): Promise<string> {
	...
	await cj.setCookie(
		`VstCnt=${encodeURIComponent(btoa(lastIndex))}; path=/; secure`,
		psaUrl.origin
	);

	try {
		const { headers } = await axiosInstance.get(exitLink, {
			jar: cj,
			maxRedirects: 0,
			validateStatus: (code) => code === 302
		});
